...
Rents an RTX 4090, uploads the PDF + pipeline scripts,
...
SCRIPTS_DIR = BASE_DIR / "cloud_pipeline"
...
def load_api_key() -> str:
    ...
    if not env_path.exists():
        log("❌ API key not found at /workspace_qnap/vast_ai/.env")
        sys.exit(1)
    with open(env_path) as f:
        for line in f:
            if "VAST_API_KEY" in line and "=" in line:
                return line.split("=", 1)[1].strip()
    log("❌ VAST_API_KEY not found in .env")
    sys.exit(1)
...
def search_gpu(gpu_name: str = DEFAULT_GPU, max_price: float = DEFAULT_MAX_PRICE) -> dict:
    ...
    cmd = (
        ...
        f'"gpu_name={gpu_name} num_gpus=1 rentable=true verified=true" '
        f'-o dph_total+ --raw --limit 10'
    )
    output = run_cmd(cmd, capture=True, timeout=60)
    if not output:
        log(f"  No verified offers. Trying unverified...")
        cmd = (
            f'vastai search offers '
    ...
    if not cheap:
        ...
        sys.exit(1)
    best = cheap[0]
    log(f"  ✅ Best offer: ID={best['id']} @ ${float(best['dph_total']):.4f}/hr")
    return best


def create_instance(offer_id: int) -> str:
    log(f"🚀 Creating instance from offer {offer_id}...")
    cmd = (
        f'vastai create instance {offer_id} '
        f'--image "{DOCKER_IMAGE}" '
        f'--disk {DISK_GB} --ssh --direct --raw'
    )
    output = run_cmd(cmd, capture=True, timeout=120)
    data = json.loads(output) if output else {}
    instance_id = data.get("new_contract", "")
    if not instance_id:
        log(f"❌ Failed to create instance: {data}")
        sys.exit(1)
...
def wait_for_running(instance_id: str) -> dict:
    log(f"⏳ Waiting for instance {instance_id} to be ready...")
    start = time.time()
    ...
    while time.time() - start < max_wait:
        ...
        if status == "running":
            ...
            log(f"  ✅ Instance running! SSH: root@{ip}:{port}")
            ...
            sys.exit(1)
        time.sleep(POLL_INTERVAL)
    log(f"⏱️  Timeout after {MAX_WAIT_MIN} min")
    destroy_instance(instance_id)
    sys.exit(1)
...
def upload_to_instance(ssh_info: dict, local_path: Path, remote_path: str = None):
    if not local_path.exists():
        log(f"  ⚠️  Local path not found: {local_path}")
        return
...
def exec_on_instance(ssh_info: dict, command: str, timeout: int = 3600) -> str:
    ...
    log(f"  ⚙️  Executing (timeout={timeout}s)...")
    result = run_cmd(cmd, capture=True, timeout=timeout)
    lines = result.split("\n")
    if len(lines) > 20:
        log(f"     ... ({len(lines)} total lines, showing last 20)")
        for l in lines[-20:]:
            if l.strip():
                log(f"     {l}")
    else:
...
def download_from_instance(ssh_info: dict, remote_path: str, local_path: Path):
    ...
    log(f"  📥 Downloaded: {len(files)} files ({size/1e6:.1f} MB) → {local_path}")
...
def destroy_instance(instance_id: str):
    ...
    log(f"  ✅ Instance destroyed")


def main():
    parser = argparse.ArgumentParser(description="Orchestrate PDF ingestion on vast.ai GPU")
    parser.add_argument("--pdf", default="Brophy.pdf", help="PDF filename in pdf_input/")
    parser.add_argument("--gpu", default=DEFAULT_GPU, help=f"GPU model")
    ...
    log(f"PDF: {pdf_path.name} ({pdf_size_mb:.0f} MB), GPU: {args.gpu} (max ${args.max_price}/hr)")

    if args.dry_run:
        log("🏁 Dry run — exiting without renting")
        return

    est_cost = (args.max_price / 60) * 25
    try:
        input(f"\n🚀 Est. cost ~${est_cost:.2f}. Press Enter to launch, Ctrl+C to cancel...")
    ...
    try:
        ...
        log("📤 Uploading pipeline files...")
        upload_to_instance(ssh_info, pdf_path, "/workspace/input/")
        upload_to_instance(ssh_info, SCRIPTS_DIR / "run_pipeline.sh", "/workspace/run_pipeline.sh")
        upload_to_instance(ssh_info, SCRIPTS_DIR / "vlm_local.py", "/workspace/vlm_local.py")
        log("⚙️  Running pipeline...")
        exec_on_instance(ssh_info, "chmod +x /workspace/run_pipeline.sh && bash /workspace/run_pipeline.sh")
        ...
        pdf_path.unlink()
        log(f"✅ Done! Results: {RESULT_DIR / pdf_path.stem}")
    except KeyboardInterrupt:
        log("\n⏹️  Interrupted")
